...
assert(sandbox.notificationBadgeCount({ notificationCount: 1, smsNotificationCount: 1, hasSms: true }, true) === 0, "hidden sms is not a badge")
assert(sandbox.notificationBadgeCount({ notificationCount: 2, smsNotificationCount: 1, hasSms: true }, true) === 1, "other notifications still badge")
assert(sandbox.notificationBadgeCount({ notificationCount: 1, smsNotificationCount: 1, hasSms: true }, false) === 1, "shown sms still badges")
assert(sandbox.actionRows({ paired: true, reachable: true, notificationCount: 1, smsNotificationCount: 1, hasSms: true }, true)[0].label === "Notifications", "pad label drops a lone sms count")
...
assert(sandbox.mediaState({ paired: true, reachable: true }) === null, "no media state without payload")
assert(sandbox.mediaState({ paired: true, reachable: true, media: { hasMedia: false, title: "" } }) === null, "no media rows when idle")
...
assert(sandbox.moveActionIndex(mediaRows, 0, 1, 0) === 1 && sandbox.moveActionIndex(mediaRows, 2, 1, 0) === 2, "media row navigation")
assert(sandbox.mediaMeta(playing.media) === "Artist · Album", "media meta joins artist and album")
assert(sandbox.mediaMeta({ title: "Song" }) === "", "media meta empty without artist")
assert(sandbox.typeIcon("tablet") === "󰓹", "tablet icon")
assert(sandbox.typeIcon("smartphone") === "󰄜", "phone icon")
assert(sandbox.looksLikePhone("801-555-0100") === true, "phone-like")
assert(sandbox.looksLikePhone("Ada") === false, "name is not a phone")
